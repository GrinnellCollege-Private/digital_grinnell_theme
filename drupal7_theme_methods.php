...
/**
 * Get rid of 'Home' in breadcrumb trail for the "Digital Grinnell" theme.
 *
 * Lifted from http://stackoverflow.com/questions/3579742/how-do-i-remove-home-from-breadcrumb
 *
 */
...
/* MAM replaced code below with block above on 06-August-2015.
function Digital_Grinnell_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
...
function Digital_Grinnell_preprocess_page(&$vars) {
  ...
  if (!module_exists('dg7')) { return; }

  $uri = request_uri();
  $address_parts = explode('/', $uri);
  if (count($address_parts) > 3 && $address_parts[1] === 'islandora' && $address_parts[2] === 'object') {
    ...
    // Modified by MAM 04-Dec-2018 so that URLs that have no trailing '?' do not produce invalid offset notices.
    if (strpos($pid, '?') !== false) { list($pid, $garbage) = explode('?', $pid); }
    ...
    $vars['stats'] = dg7_print_awstats($pid);
  }

  // Remove the "View" tab if there are NO others to be shown.  MAM added on 16-Oct-2015

  if (!isset($vars)) { return; }
  if (!isset($vars['tabs'])) { return; }
  if (!is_array($vars['tabs'])) { return; }

  $others = FALSE;

  foreach ($vars['tabs'] as $tab) {
    if (isset($tab['#theme']) && $tab['#theme'] === 'menu_local_task') {
  ...
  if (!$others) {
    foreach ($vars['tabs'] as $index => $tab) {
      if ((isset($tab['#theme']) && $tab['#theme'] === 'menu_local_task') &&
        (isset($tab['#link']['title']) && $tab['#link']['title'] === 'View')) {
        unset($vars['tabs'][$index]);
        return;
      }
    }
  }
